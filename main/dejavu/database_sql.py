...
class SQLDatabase(Database):
    ...
    INSERT_FINGERPRINT = """INSERT INTO %s (%s, %s, %s) values (UNHEX(\"%%s\"), %%s, %%s);""" % (FINGERPRINTS_TABLENAME, Database.FIELD_HASH, Database.FIELD_SONG_ID, Database.FIELD_OFFSET)
    ...
    def __init__(self, **options):
        ...
        leveldbe = db_class("LevelDB")
        self.leveldb = leveldbe("fingerPrints", False)
    ...
    def insert(self, hash, sid, offset):
        ...
        """
        with self.cursor() as cur:
            cur.execute(self.INSERT_FINGERPRINT, (hash, int(sid), int(offset)))
    ...
    def insert_hashes(self, sid, hashes):
        ...
        """
        wb = self.leveldb.write_batch()
        with self.leveldb.write_batch() as b:
            for hash, offset in hashes:
                value = str(sid)+ "|" + str(offset)
                b.put(hash,value.encode(encoding='utf_8'))
                # values.append((hash, int(sid), int(offset)))
    ...
    def return_matches(self, hashes):
        ...
        for split_values in grouper(values, 1000):
            ...
            for j in split_values:
                v = self.leveldb.get(j)
                if(v):
                    sp = v.decode("utf-8").split("|")
                    cur.append((j, int(sp[0]), int(sp[1])))
            for xk in cur:
                # (sid, db_offset - song_sampled_offset)
                hash, sid, offset = xk
                yield (sid, offset - int(mapper[hash]))
